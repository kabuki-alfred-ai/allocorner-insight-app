...
export async function createInvitation(
 ...
 const response = await apiClient.post<Invitation>(`/projects/${projectId}/invitations`,
 data,
 );
...
export async function validateInvitation(
 ...
 const response = await axios.get<Invitation>(`${baseURL}/invitations/${token}`,
 );
 return response.data;
}
...
export async function acceptInvitation(
 ...
 const baseURL =
 import.meta.env.VITE_API_URL || 'http://localhost:3000/api';
 const response = await axios.post<AuthResponse>(`${baseURL}/invitations/${token}/accept`,
 data,
 );
 return response.data;
}
