...
export const VerbatimScene: React.FC<{ verbatims: VerbatimData[]; defaultSceneLength: number; theme: WrappedTheme }> = ({ verbatims, defaultSceneLength, theme }) => {
...
const SingleVerbatim: React.FC<VerbatimData & { verbatimIndex?: number; totalVerbatims?: number; theme: WrappedTheme }> = ({
  text, authorRank, emotion, audioUrl, durationInFrames = 210, verbatimIndex = 0, totalVerbatims = 1, theme
}) => {
    ...
    <div style={{ opacity: fadeOut }} className="flex-1 flex flex-col items-center justify-center p-8 md:p-16 h-full w-full relative overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-around h-full px-6 gap-1.5">
          {[...Array(50)].map((_, i) => {
            ...
            return (
              <div
                key={i}
                className="w-full rounded-t-full animate-waveform"
                style={{
      ...
      <div className="absolute inset-0 opacity-15 pointer-events-none flex items-center justify-center">
        <div
          ...
          style={{
            background: `radial-gradient(ellipse at center, ${hexToRgba(theme.primaryColor, 0.2)} 0%, transparent 60%)`,
      ...
      {/* Counter badge */}
      {totalVerbatims > 1 && (
        <div className="absolute top-8 right-8 md:top-12 md:right-12 z-20" style={{ opacity: counterOpacity }}>
      ...
      <div className="max-w-5xl mx-auto w-full z-10 flex flex-col items-center text-center" style={{ opacity: fadeIn }}>
        ...
        {/* Quote Icon */}
        <div style={{ transform: `scale(${iconScale}) rotate(${iconRotation}deg)` }} className="mb-10">
          <Quote className="h-14 w-14 md:h-20 md:w-20 opacity-70" style={{ color: theme.primaryColor }} />
        </div>

        {/* Word-by-word reveal */}
        <div className="w-full px-4 md:px-8">
          <p className={`${getFontSizeClass(text.length)} font-semibold leading-snug tracking-tight`} style={{ color: theme.textColor }}>
            <span className="italic" style={{ color: theme.primaryColor }}>"</span>
            {words.map((word, wi) => {
              ...
              return (
                <span
                  key={wi}
                  style={{ opacity: wordOpacity, transform: `translateY(${wordY}px)`, display: 'inline-block', marginRight: '0.3em' }}
                  className="italic"
                >
                  {word}
                </span>
        ...
        >
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4">
            <span className="text-lg md:text-xl font-medium" style={{ color: hexToRgba(theme.textColor, 0.8) }}>
              {authorRank || "Participant"}
            </span>
            {emotion && (
              <>
                <span className="hidden md:block w-1.5 h-1.5 rounded-full" style={{ backgroundColor: theme.primaryColor }} />
                <span className="text-sm md:text-base font-semibold" style={{ color: `${theme.primaryColor}b3` }}>
                  {emotion}
                </span>
              </>
            )}
