...
const Particles: React.FC<{ count?: number; theme: WrappedTheme }> = ({ count = 30, theme }) => {
        ...
        return (
          <div
            ...
            style={{
              ...
              height: size,
              borderRadius: '50%',
              backgroundColor: `hsl(${hsl.h + (random(`${seed}-hue`) - 0.5) * 20}, ${hsl.s}%, ${50 + random(`${seed}-l`) * 25}%)`,
              opacity: pulseOpacity + random(`${seed}-op`) * 0.2,
              filter: `blur(${random(`${seed}-bl`) * 1.5}px)`,
              boxShadow: size > 4 ? `0 0 ${size * 3}px ${hexToRgba(theme.primaryColor, 0.25)}` : 'none',
            }}
          />
        );
      })}
...
const ExpandingRings: React.FC<{ theme: WrappedTheme }> = ({ theme }) => {
  const frame = useCurrentFrame();
  const ringCount = 3;
...
export const IntroScene: React.FC<{ clientName: string; projectName: string; theme: WrappedTheme }> = ({
  clientName,
  projectName,
  theme,
}) => {
  ...
  const projectScale = spring({ frame: frame - projectDelay, fps, config: { damping: 7, stiffness: 120, mass: 0.9 } });
  ...
  return (
    <div
      style={{ opacity: fadeOut, transform: `scale(${scaleOut})`, backgroundColor: theme.bgColor, color: theme.textColor }}
      className="flex-1 flex flex-col items-center justify-center text-center h-full w-full relative overflow-hidden"
    >
      {/* Center warm glow */}
      <div className="absolute inset-0 pointer-events-none" style={{ opacity: auraOpacity * 0.5 }}>
        <div
          style={{
            position: 'absolute', left: '50%', top: '50%',
            transform: `translate(-50%, -50%) scale(${glow1Scale * auraPulse})`,
            width: '140%', height: '140%', borderRadius: '50%',
            background: `radial-gradient(circle at center, ${hexToRgba(theme.primaryColor, 0.18)} 0%, ${hexToRgba(theme.primaryColor, 0.06)} 35%, transparent 60%)`,
          }}
        />
      </div>

      {/* Accent drift top right */}
      <div className="absolute inset-0 pointer-events-none" style={{ opacity: auraOpacity * 0.3 }}>
        <div
          style={{
            position: 'absolute', left: `${glow2X}%`, top: `${glow2Y}%`,
            transform: 'translate(-50%, -50%)', width: '50%', height: '50%',
            borderRadius: '50%',
            background: `radial-gradient(circle, ${hexToRgba(theme.primaryColor, 0.15)} 0%, transparent 60%)`,
            filter: 'blur(40px)',
          }}
        />
      </div>
      ...
      {/* Bottom warm pool */}
      <div className="absolute inset-0 pointer-events-none" style={{ opacity: auraOpacity * 0.25 }}>
        <div
          style={{
            position: 'absolute', left: '30%', top: `${glow3Y}%`,
            transform: 'translate(-50%, -50%)', width: '60%', height: '40%',
            borderRadius: '50%',
            background: `radial-gradient(ellipse, ${hexToRgba(theme.primaryColor, 0.12)} 0%, transparent 60%)`,
            filter: 'blur(50px)',
          }}
        />
      </div>
      ...
      <div className="z-10 w-full flex flex-col items-center px-8">
        ...
        <div style={{ opacity: clientFadeIn }} className="mb-5 h-16 flex items-center justify-center">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-medium" style={{ letterSpacing: '0.5em' }}>
              ...
              return (
                <span
                  ...
                  style={{
                    ...
                    color: isLatest ? theme.textColor : theme.primaryColor,
                    textShadow: isLatest
                      ? `0 0 20px ${hexToRgba(theme.textColor, 0.6)}`
                      : `0 0 15px ${hexToRgba(theme.primaryColor, 0.3)}`,
            ...
            {!typewriterDone && (
              <span
                ...
                style={{
                  ...
                  boxShadow: `0 0 8px ${hexToRgba(theme.primaryColor, 0.6)}`,
                }}
              />
            )}
          </h2>
        ...
        <div className="relative h-[2px] mb-10 overflow-hidden" style={{ width: 180 }}>
          <div
            ...
            style={{
              ...
              background: `linear-gradient(to right, transparent, ${hexToRgba(theme.primaryColor, 0.7)}, transparent)`,
              boxShadow: lineProgress > 0.5 ? `0 0 12px ${hexToRgba(theme.primaryColor, 0.3)}` : 'none',
            }}
          />
        </div>
        ...
        {/* Project name */}
        <div
          style={{ opacity: projectOpacity, transform: `scale(${projectScale})`, filter: `blur(${projectBlur}px)` }}
          className="mb-6 relative"
        >
          <h1
            className="text-7xl md:text-[7.5rem] lg:text-[9.5rem] font-semibold tracking-[-0.04em] leading-[0.85] absolute inset-0"
            style={{
              color: 'transparent',
              WebkitTextStroke: `1px ${hexToRgba(theme.primaryColor, 0.08)}`,
          ...
          <h1
            className="text-7xl md:text-[7.5rem] lg:text-[9.5rem] font-semibold tracking-[-0.04em] leading-[0.85] relative"
            style={{
              backgroundImage: `linear-gradient(${gradientAngle}deg, ${theme.textColor} 0%, ${hexToRgba(theme.textColor, 0.85)} 40%, ${hexToRgba(theme.textColor, 0.5)} 100%)`,
              backgroundClip: 'text',
        ...
        {/* Subtitle */}
        <div style={{ opacity: subtitleOpacity, transform: `translateY(${subtitleY}px)` }} className="mt-10">
          <p className="text-xl md:text-2xl font-medium" style={{ letterSpacing: `${subtitleLetterSpacing}em`, color: hexToRgba(theme.textColor, 0.35) }}>
            Le projet en revue
          </p>
        </div>
      </div>
