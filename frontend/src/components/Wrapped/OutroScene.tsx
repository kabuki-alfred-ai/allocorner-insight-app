...
const ExplodingParticles: React.FC<{ count?: number; theme: WrappedTheme }> = ({ count = 35, theme }) => {
        ...
        return (
          <div
            ...
            style={{
              ...
              backgroundColor: `hsl(${hsl.h + (random(`${seed}-hue`) - 0.5) * 20}, ${hsl.s}%, ${50 + random(`${seed}-light`) * 25}%)`,
              opacity: particleOpacity,
              filter: `blur(${random(`${seed}-blur`) * 1}px)`,
              boxShadow: `0 0 ${size * 2}px ${hexToRgba(theme.primaryColor, 0.4)}`,
            }}
          />
        );
      })}
...
export const OutroScene: React.FC<{ clientName: string; theme: WrappedTheme }> = ({ clientName, theme }) => {
  const frame = useCurrentFrame();
  ...
  return (
    <div
      style={{ opacity: fadeOut, transform: `scale(${convergeScale})`, backgroundColor: theme.bgColor, color: theme.textColor }}
      className="flex-1 flex flex-col items-center justify-center p-8 text-center h-full w-full relative overflow-hidden"
    >
      <ExplodingParticles count={35} theme={theme} />

      {/* Background Aura */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ opacity: auraOpacity }}>
        <div
          style={{
            width: '180%', height: '180%', borderRadius: '50%',
            background: `radial-gradient(circle at center, ${hexToRgba(theme.primaryColor, 0.2)} 0%, ${hexToRgba(theme.primaryColor, 0.05)} 35%, transparent 60%)`,
      ...
      <div className="z-10 flex flex-col items-center">
        ...
        {/* Animated check icon */}
        <div className="mb-10" style={{ transform: `scale(${iconSpring})` }}>
          <div
            className="w-24 h-24 md:w-28 md:h-28 rounded-full flex items-center justify-center"
            style={{
              background: `linear-gradient(135deg, ${hexToRgba(theme.primaryColor, 0.15)} 0%, ${hexToRgba(theme.primaryColor, 0.05)} 100%)`,
              border: `2px solid ${hexToRgba(theme.primaryColor, 0.2)}`,
              boxShadow: `0 0 ${40 + iconGlow * 30}px ${hexToRgba(theme.primaryColor, 0.1 + iconGlow * 0.2)}`,
            }}
          >
        ...
        {/* Thank you text */}
        <div style={{ opacity: textOpacity, transform: `scale(${textSpring})`, filter: `blur(${textBlur}px)` }}>
          <h2 className="text-5xl md:text-7xl lg:text-8xl font-semibold tracking-tighter mb-3 leading-[0.9]">
            <span
              className="text-transparent bg-clip-text"
              style={{ backgroundImage: `linear-gradient(135deg, ${theme.textColor} 0%, ${hexToRgba(theme.textColor, 0.85)} 50%, ${hexToRgba(theme.textColor, 0.5)} 100%)` }}
            >
              Merci,{' '}
            </span>
            <br />
            <span
              className="text-transparent bg-clip-text"
              style={{ backgroundImage: `linear-gradient(to right, ${theme.primaryColor}, ${theme.primaryColor}99)` }}
            >
              {clientName}.
            </span>
          </h2>
        ...
        <div style={{ opacity: subtitleOpacity, transform: `translateY(${subtitleY}px)` }} className="mt-6">
          <p className="text-xl md:text-2xl font-medium tracking-[0.3em]" style={{ color: hexToRgba(theme.textColor, 0.4) }}>
            L'écoute continue.
          </p>
        </div>
        ...
        <div style={{ opacity: brandOpacity }} className="mt-16">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-semibold tracking-[0.5em]" style={{ color: hexToRgba(theme.textColor, 0.2) }}>
              AlloCorner Insight
            </span>
            <div className="w-2 h-2 rounded-full" style={{ backgroundColor: theme.primaryColor }} />
          </div>
