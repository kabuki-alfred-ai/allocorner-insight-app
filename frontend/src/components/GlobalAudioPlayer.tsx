...
export function GlobalAudioPlayer() {
 ...
 <p className="text-base font-semibold text-foreground truncate">{currentMessage.speaker ||"Anonyme"}</p>
 <span className="text-[10px] font-medium text-black/20 tracking-tighter truncate max-w-[150px]">{currentMessage.filename}</span>
 </div>
 </div>
 ...
 <span className="text-[10px] font-semibold text-primary/60 tabular-nums w-8">{formatTime(currentTime)}</span>
 <Slider
 value={[progress]}
 ...
 className="flex-1"
 />
 <span className="text-[10px] font-semibold text-black/20 tabular-nums w-8">{formatTime(duration)}</span>
 </div>
 </div>
 </div>
 </div>
